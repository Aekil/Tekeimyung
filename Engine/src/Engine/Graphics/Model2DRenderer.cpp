                                            ...
                                            uint32_t height)
{
    ...
    if (!planeEntityRender)
    {
        LOG_ERROR("Model2DRenderer::renderModelOnPlane : Can't get sRenderComponent on plane entity %s", planeEntity->getTag().c_str());
        return (false);
    }
    ...
    if (!planeEntityModelInstance->getMeshsInstances().size())
    {
        LOG_ERROR("Model2DRenderer::renderModelOnPlane : The plane entity %s has no mesh", planeEntity->getTag().c_str());
        return (false);
    }
